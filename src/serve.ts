...
export function makeRequestHandler(source: any, { principalOf = anonymousPrincipalOf, db, env = config.env, rateLimiter = null, csp, hsts, cors, requestLog = false, authorization }: {
  ...
  const handlers = [
    ...
    {
      match: (req: any, url: URL) => isApp && req.method === 'GET' && url.pathname === '/health',
      handle: (_req: any, res: any) => { sendJson(res, 200, { status: 'ok', env }); return true; },
    },
    // /health/stats — includes uptime, RSS, request count
    {
      match: (req: any, url: URL) => isApp && req.method === 'GET' && url.pathname === '/health/stats',
    ...
    {
      match: (req: any, _url: URL) => isApp && req.method === 'OPTIONS' && cors && cors.origins && Array.isArray(cors.origins),
    ...
    {
      match: (req: any, _url: URL) => isApp && req.method === 'GET',
    ...
    {
      match: (req: any, _url: URL) => isApp && req.method === 'POST',
      handle: async (req: any, res: any) => {
    ...
    {
      match: (req: any, _url: URL) => isApp && req.method === 'POST' && source.jobs,
      handle: async (req: any, res: any) => {
    ...
    {
      match: (_req: any, _url: URL) => isApp,
      handle: async (req: any, res: any) => handleApplicationActionHttp(source, req, res, actionPrincipalOf, sendJsonCompat),
    },
    ...
    {
      match: (_req: any, _url: URL) => isApp && Boolean(source._applicationLiveDelivery),
      handle: async (req: any, res: any) => {
    ...
    {
      match: (_req: any, _url: URL) => isApp && source._handlers?.length,
      handle: async (req: any, res: any, url: URL) => {
